...
const GuestOrderLookup = () => {
    ...
    const handleLookup = async (e) => {
        e.preventDefault();
        if (!orderId || !phone) {
            addToast('주문번호와 연락처를 모두 입력해 주세요.');
            return;
        }

        setLoading(true);
        const result = await fetchGuestOrder(orderId.trim(), phone.trim());
        setLoading(false);
    ...
    return (
        <main className="max-w-4xl mx-auto px-4 py-8 mb-20 min-h-[60vh]">
            ...
            <div className="mb-10 text-center">
                ...
                <p className="text-slate-500">주문번호와 연락처를 입력하면 주문 내역을 조회할 수 있습니다.</p>
            </div>

            <div className="max-w-md mx-auto mb-16 bg-white rounded-2xl p-8 shadow-sm border border-slate-100">
                <form onSubmit={handleLookup} className="space-y-5">
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">주문번호</label>
                        <input
                            type="text"
                            placeholder="20260326-1543-XXXX"
                            value={orderId}
            ...
            {!orders.length && !loading && (
                <div className="mt-12 text-center p-10 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                    ...
                    <p className="text-slate-500 text-sm">아직 조회된 내역이 없습니다. 주문번호와 연락처를 입력해 주세요.</p>
                </div>
            )}
