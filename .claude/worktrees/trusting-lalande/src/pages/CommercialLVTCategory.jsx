...
export default function CommercialLVTCategory() {
    ...
    const filteredProducts = useMemo(() => {
        ...
        if (detailTab === '전체') {
            ...
            if (detailTab === '사각600각') {
                result = result.filter(p => p.title?.includes('600각') || p.specifications?.size?.includes('600'));
            } else if (detailTab === '사각450각') {
                result = result.filter(p => p.title?.includes('450각') || p.specifications?.size?.includes('450'));
            } else if (detailTab === '우드') {
                result = result.filter(p => p.title?.includes('우드') || p.patterns?.includes('Wood') || p.model_id?.includes('W'));
            }
        } else if (activeTab === 'LVT 프리미엄 5T(프레스티지)') {
            if (detailTab === '사각600각') {
                result = result.filter(p => (p.model_id?.startsWith('PTT') || p.title?.includes('사각')) && p.specifications?.size?.includes('600'));
            } else if (detailTab === '사각') {
                result = result.filter(p => (p.model_id?.startsWith('PTT') || p.title?.includes('사각')) && !p.specifications?.size?.includes('600'));
            } else if (detailTab === '우드') {
                result = result.filter(p => p.model_id?.startsWith('PTW') || p.patterns?.includes('Wood') || p.title?.includes('우드'));
            }
        }
                                    ...
                                    <div className="mt-4">
                                        <div className="flex items-baseline gap-1.5">
                                            {isAuthenticated && isBusiness ? (
                                                <span className="text-[18px] xl:text-[20px] font-black text-[#c0392b]">
                                                    {product.businessPrice?.toLocaleString()}원~
                                                </span>
                                            ) : (
                                                <span className="text-[18px] xl:text-[20px] font-black text-slate-900">
                                                    {product.price?.toLocaleString()}원~
                                                </span>
                                            )}
                                            <span className="text-[12px] text-slate-400 font-medium">/{product.priceUnit || 'm²'}</span>
                                        </div>
                                    </div>
                                </div>
